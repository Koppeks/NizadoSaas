...
export const CalendarMini = forwardRef<HTMLDivElement, CalendarProps>(
  ({ disableDays, events, ...props }, ref) => {
    ...
    const renderDay = (day: number, disabled: boolean) => {
      const filteredDays = events?.filter((event) => {
        if (event.repetition && event.eventType === "REPETITION") {
          return event.repetition.repeatedDays.includes(
      ...
      return (
        ...
        >
          <Text
            as="p"
            className={`day ${!disabled && "not_from_current_month"}`}
          >
            {day}
          </Text>
          {disabled &&
    ...
    return (
      <div className="Calendar_Mini_Component">
